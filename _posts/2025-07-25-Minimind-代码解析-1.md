...
```
        ...
        xv = xv.view(bsz, seq_len, self.n_local_kv_heads, self.head_dim)
```
由这段代码可以看出输入张量 $x$ 的形状为 $(batch\_size, seq\_len, dim)$。经过线性投影后得到 $xq、xk、xv$，形状变为 $(batch\_size, seq\_len, hidden\_dim)$，通过 view 方法，我们将 $xq、xk、xv$ 看成形状为 $(batch\_size, seq\_len, num\_heads, head\_dim)$，从而便于执行多头注意力机制。
```
        cos, sin = position_embeddings
        xq, xk = apply_rotary_pos_emb(xq, xk, cos[:seq_len], sin[:seq_len])
```
...
```
将 $xq、xk、xv$ 的形状分别转换为 $(batch\_size, num\_heads, seq\_len, head\_dim)、(batch\_size, num\_heads * num\_repeat, seq\_len, head\_dim)、(batch\_size, num\_heads * num\_repeat, seq\_len, head\_dim)$
