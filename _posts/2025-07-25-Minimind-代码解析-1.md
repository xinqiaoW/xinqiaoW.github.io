...
```
        ...
        xv = xv.view(bsz, seq_len, self.n_local_kv_heads, self.head_dim)
```
由这段代码可以看出输入张量 $x$ 的形状为 $(batch_size, seq_len, dim)$。经过线性投影后得到 $xq、xk、xv$，形状变为 $(batch_size, seq_len, hidden_dim)$，通过 view 方法，我们将 $xq、xk、xv$ 看成形状为 $(batch_size, seq_len, num_heads, head_dim)$，从而便于执行多头注意力机制。
```
        cos, sin = position_embeddings
        xq, xk = apply_rotary_pos_emb(xq, xk, cos[:seq_len], sin[:seq_len])
```
...
```
将 $xq、xk、xv$ 的形状分别转换为 $(batch_size, num_heads, seq_len, head_dim)、(batch_size, num_heads * num_repeat, seq_len, head_dim)、(batch_size, num_heads * num_repeat, seq_len, head_dim)$
